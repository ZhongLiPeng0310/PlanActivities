      ...
      <ul class="book-list">
        <li class="book-list-item">
          ...
          <div class="book-count-info">
            ...
            <img v-show="payCommList.imagePath" :src="payCommList.imagePath" >
            <img v-show="!payCommList.imagePath" src="../../assets/timg.png">
            <div>
              <div>{{payCommList.goodsName}}</div>
              <div>圆若骊珠，赤若金丸，肉似玻璃，核如黑漆。</div>
              <!-- <div>{{item.goodsIntroduce}}</div> -->  <!-- 此处是介绍 -->
              <div>
                <span>￥{{payCommList.salePrice}}</span>
                <div class="change-count-box">
                  <div>共{{payCommList.orderSum}}件</div>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  data () {
    return {
      ...
      getList: [],
      totalPrice: 0,
      payCommList: JSON.parse(sessionStorage.getItem('currentComm')),
  ...
  methods: {
    ...
    DwAdd () {
      console.log(111)
      this.$router.push({path: '/location-info3'
        , query: {
