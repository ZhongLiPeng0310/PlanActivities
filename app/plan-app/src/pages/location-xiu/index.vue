<template>
  <div>
   <el-form ref="form" :model="formData" :rules="formRules" inline label-width="80px">
      ...
      <el-form-item label="是否默认" prop="betrue">
        <el-select v-model="formData.betrue">
          <el-option
          v-for="(item, index) in betrueOptions"
          :key="index"
          :label="item.label"
      ...
      <el-form-item>
        <el-button class="register-btn" type="primary" @click="submit">确&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;定</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      formRules: {
        ...
        betrue: [
          {required: true, message: '请选择是否默认该地址', trigger: 'change'}
        ]
      },
      betrueOptions: [
        {label: '默认地址', value: '0'},
        {label: '不默认该地址', value: '1'},
      ],
    }
  },
  computed: {
  ...
  methods: {
    proId () {
      ...
      console.log('this.$route.query.xiuLoca', this.$route.query.xiuLoca)
      if (this.$route.query.xiuLoca[0]) { // 赋值
        Promise.all([
          this.citId(this.$route.query.xiuLoca[0].provinceCode),
          this.areId(this.$route.query.xiuLoca[0].cityCode)
        ]).then(dataList => {
          this.formData = Object.assign({}, this.$route.query.xiuLoca[0], {
            phone: Number(this.$route.query.xiuLoca[0].phone),
            betrue: ''
          })
        })
      }
